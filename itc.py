...
from coclust import coclustering
...
def form_biclusters(data_matrix, categories):
    bicluster = coclustering.CoclustInfo(n_row_clusters=len(categories), n_col_clusters=len(categories))
...
def divide_biclusters(bicluster, categories, data_matrix, document_names, feature_names):
    ...
    for id_ in range(len(categories)):
        n_rows, n_cols = bicluster.get_shape(id_, id_)
        cluster_docs  = bicluster.get_row_indices(id_)
        cluster_words = bicluster.get_col_indices(id_)
        cat = defaultdict(int)
...
if __name__ == '__main__':
    ...
    # a = np.zeros(data_matrix.shape)
    # print(type(a))
    # b = data_matrix.todense()
    # b = np.asarray(b)
    ...
    #     np.savetxt(f, a, delimiter=' ', fmt='%i')
